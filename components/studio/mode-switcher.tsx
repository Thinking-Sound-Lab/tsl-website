...
export function ModeSwitcher() {
  ...
  return (
    <div className="flex flex-col gap-1 bg-secondary/30 rounded-xl p-1 self-end">
      {modes.map(({ value, icon, label }) => (
        <button
          key={value}
          onClick={() => setMode(value)}
          className={cn(
            "flex flex-col items-center gap-1 px-2 py-2 rounded-lg text-[10px] font-medium transition-colors cursor-pointer w-14",
            mode === value
